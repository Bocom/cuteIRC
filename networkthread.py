...
class NetworkThread(QThread):
  ...
  def __init__(self, parent, net):
    ...
    if serverpass:
      self.send("PASS %s" % serverpass)
    self.start()
  
  def attach_window(self, wid, window):
    pass
  
  def detach_window(self, wid):
    pass
  ...
  def send(self, data):
    data_len = len(data)
    ret = self.socket.send(bytes(data, "utf-8"))
    if ret < data_len:
      print("Command not sent correctly")
...
class IRCEvent:
  def __init__(self, command, source, target, arguments, message):
...
class CTCPEvent(IRCEvent):
  def __init__(self, command, source, target, ctcp_command, message):
    IRCEvent.__init__(self, command, source, [], target, message)
    self.ctcp_command = ctcp_command
